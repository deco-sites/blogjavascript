...
const blogPostPattern = new URLPattern(`/post/:blogpost`, window.origin);
...
window.addEventListener('pageswap', async (e) => {
	...
	if (e.viewTransition) {
        ...
        if (isBlogPostPage(targetUrl)) {
            const blogpost = extractBlogPostNameFromUrl(targetUrl).replaceAll(".", "");
...
window.addEventListener('pagereveal', async (e) => {
	...
	if (e.viewTransition) {
		...
		if (isBlogPostPage(fromUrl) && isHomePage(currentUrl)) {
			const blogpost = extractBlogPostNameFromUrl(fromUrl).replaceAll(".", "");
